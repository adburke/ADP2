 ...
 */
public class MyTasksFragment extends ListFragment implements ParseQueryAdapter.OnQueryLoadListener<ParseObject> {
                ...
                filterBuilder.setTitle(R.string.action_filter)
                        .setItems(R.array.filters, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                switch (which) {
                                    case 0:
                                        taskListAdapter = new TaskListAdapter(mContext,null, "All");
                                        filter = false;
                                        updateList(taskListAdapter);
                                        break;
                                    case 1:
                                        taskListAdapter = new TaskListAdapter(mContext,null, "Active");
                                        filter = true;
                                        updateList(taskListAdapter);
                                        break;
                                    case 2:
                                        taskListAdapter = new TaskListAdapter(mContext,null, "Completed");
                                        filter = true;
                                        updateList(taskListAdapter);
                                        break;
                                }
